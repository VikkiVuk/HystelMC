...
public class TileEntityRainbowChest extends TileEntityLockableLoot implements ITickable 
{
	...
	{
		return this.hasCustomName() ? this.customName : "container.rainbow_chest";
	}
	...
	{
		return new ContainerRainbowChest(playerInventory, this, playerIn);
	}
	
	@Override
	public String getGuiID() 
	{
		return Reference.MOD_ID + ":rainbow_chest";
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerRainbowChest)
                {
                    if (((ContainerRainbowChest)entityplayer.openContainer).getChestInventory() == this)
                    {
