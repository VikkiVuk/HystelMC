...
public class SitChairsyEvent
{
    ...
    public static void onInteractWithBlock(PlayerInteractEvent.RightClickBlock event)
    {
        ...
        if(state.getBlock() instanceof BlockChairsy)
        {
            //List<SeatStair> seats = worldIn.getEntitiesWithinAABB(SeatStair.class, new AxisAlignedBB(pos, pos.add(1, 1, 1)));
            SeatChairsy seat = new SeatChairsy(worldIn, pos);
            worldIn.spawnEntity(seat);
            player.startRiding(seat);
        }
    }

    public static class SeatChairsy extends Entity
    {
        public SeatChairsy(World worldIn, BlockPos pos)
        {
            this(worldIn);
            setPosition(pos.getX() + 0.5D, pos.getY() + 0.3D, pos.getZ() + 0.5D);
        }

        public SeatChairsy(World worldIn)
        {
            super(worldIn);
            setSize(0.0F, 0.0F);
        }
        ...
        public void onUpdate()
        {
            ...
            if(!(getEntityWorld().getBlockState(pos).getBlock() instanceof BlockChairsy))
            {
                setDead();
                return;
            }
