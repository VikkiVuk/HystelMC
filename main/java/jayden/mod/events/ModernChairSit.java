...
public class ModernChairSit
{
    ...
    public static void onInteractWithBlock(PlayerInteractEvent.RightClickBlock event)
    {
        ...
        if(state.getBlock() instanceof BlockModernChair)
        {
            //List<SeatStair> seats = worldIn.getEntitiesWithinAABB(SeatStair.class, new AxisAlignedBB(pos, pos.add(1, 1, 1)));
        	SeatModernChair seat = new SeatModernChair(worldIn, pos);
            worldIn.spawnEntity(seat);
            player.startRiding(seat);
        }
    }

    public static class SeatModernChair extends Entity
    {
        public SeatModernChair(World worldIn, BlockPos pos)
        {
            this(worldIn);
            setPosition(pos.getX() + 0.5D, pos.getY() + 0.3D, pos.getZ() + 0.5D);
        }

        public SeatModernChair(World worldIn)
        {
            super(worldIn);
            setSize(0.0F, 0.0F);
        }
        ...
        public void onUpdate()
        {
            ...
            if(!(getEntityWorld().getBlockState(pos).getBlock() instanceof BlockModernChair))
            {
                setDead();
                return;
            }
