...
public class TileEntityBluestoneChest extends TileEntityLockableLoot implements ITickable
{
	...
	{
		return this.hasCustomName() ? this.customName : "container.bluestone_chest";
	}
	...
	{
		return new ContainerBluestoneChest(playerInventory, this, playerIn);
	}
	
	@Override
	public String getGuiID() 
	{
		return Reference.MOD_ID + ":bluestone_chest";
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerBluestoneChest)
                {
                    if (((ContainerBluestoneChest)entityplayer.openContainer).getChestInventory() == this)
                    {
