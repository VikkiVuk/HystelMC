...
public class TileEntityTokenAtm extends TileEntityLockableLoot implements ITickable
{
	...
	{
		return this.hasCustomName() ? this.customName : "container.token_atm";
	}
	...
	{
		return new ContainerTokenAtm(playerInventory, this, playerIn);
	}
	
	@Override
	public String getGuiID() 
	{
		return Reference.MOD_ID + ":token_atm";
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerTokenAtm)
                {
                    if (((ContainerTokenAtm)entityplayer.openContainer).getChestInventory() == this)
                    {
        ...
        if (this.numPlayersUsing > 0 && this.lidAngle == 0.0F)
        {
            ...
            this.world.playSound((EntityPlayer)null, d1, (double)pos.getY() + 0.5D, d2, SoundEvents.BLOCK_IRON_DOOR_OPEN, SoundCategory.BLOCKS, 0.5F, this.world.rand.nextFloat() * 0.1F + 0.9F);
        }
